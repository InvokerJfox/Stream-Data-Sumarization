...
public class SG_Main {
	...
	private static int SwapEachOne(int[] o) {
		...
		double maxg = Common.GOO(Common.GetFeather(sumZ, d));
		int maxid = -1;
		...
		for (int i = 0; i < k; i++) {
			...
			double g = Common.GOO(Common.GetFeather(tsumZ, d));
			if (g > maxg) {
				maxg = g;
				maxid = i;
			}
	...
	private static int SwapEachOneWithP(int[] o) {
		...
		double maxg = Common.GOS(sumZ, d, p);// the gain of sumZ
		int maxid = -1;

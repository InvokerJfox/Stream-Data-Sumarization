...
public class SS_Siever {
	...
	public void SieveData(int oid, int[] o) {
		...
		double og = cn.jfoxx.summarization.standalone.gain.Calculator.GOOoutF(o, f);
		if (og >= low) {// supply the demand ��insert all information
			this.Gain += og;
			f = cn.jfoxx.summarization.standalone.gain.Calculator.AddFeatherByObject(f, o);
			s_ids.add(oid);
			sumZ.add(o);
			c++;
	...
	public void SieveData(int oid, int[] o, double[] p) {
		...
		// low bound of Gain
		double low = (tGain - Gain) / (k - c);
		double og = cn.jfoxx.summarization.standalone.gain.Calculator.GOOoutF(o, f, p);
		if (og >= low) {// supply the demand ��insert all information
			this.Gain += og;
			f = cn.jfoxx.summarization.standalone.gain.Calculator.AddFeatherByObject(f, o);
			s_ids.add(oid);
